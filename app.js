...
app.post("/newEmployee", async (req, res) => {
  ...
  try {
    ...
    const startIndex = errorMessage.indexOf("ORA-20100:");
    if (startIndex !== -1) {
      const endIndex = errorMessage.indexOf("ORA-", startIndex + 1);
      const relevantMessage = errorMessage
        .substring(startIndex, endIndex)
        .trim();

      const finalMessage = relevantMessage.split(": ")[1].split("\n")[0];

      return res.status(404).json({ message: finalMessage });
    }
...
app.get("/employeeAllInfo", async (req, res) => {
  ...
  try {
    ...
    res.json({ employees: processedRows });
  } catch (err) {
